...
from auth0authorization.utils import get_decoded_token, get_user_id
from orders.models import Table, Order, OrdersProducts
...
class OrdersProductsViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, pk=None, **kwargs):
        ...
        if request_user_id != order_product.order.user.username:
            return Response(status=status.HTTP_400_BAD_REQUEST)
...
def get_active_order(pk):
    order = Order.objects.filter(pk=pk, status='In progress').first()
    return order
